...
void vmm_map_memory(uint64_t physical_address, uint64_t virtual_address) {
    ...
    for(int i = 0; i <= 2; i++) {
        ...
        if(!pt_get_flag(entry, PT_FLAG_PRESENT)) {
            ...
            pmm_set(0, (uint8_t *) new_table, 0x1000);
            uint64_t new_entry = 0;
            pt_set_address(&new_entry, (uint64_t) (uint32_t) new_table >> 12);
            ...
            current_table = (page_table_t *) (uint32_t) pt_get_address(entry);
        }
    }

    uint64_t entry = 0;
    pt_set_address(&entry, physical_address >> 12);
...
void vmm_map_memory_2mb(uint64_t physical_address, uint64_t virtual_address) {
    ...
    uint64_t entry = physical_address & 0xFFFFFFFE00000;
    pt_set_flag(&entry, PT_FLAG_PRESENT, 1);
    pt_set_flag(&entry, PT_FLAG_READ_WRITE, 1);
