...
cd binutils-build
../binutils-$BINUTILS_VERSION/configure --target=$TARGET --prefix=$PREFIX --disable-nls --disable-werror --with-sysroot --enable-multilib
make all install
cd ..
...
# Build Configure
cd ../gcc-build
../gcc-$GCC_VERSION/configure --target=$TARGET --prefix=$PREFIX --disable-nls --enable-languages=c --without-headers --with-multilib-list=m32,m64
